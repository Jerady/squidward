...
public class SplashWindow extends Window {
	private static final long serialVersionUID = -8688290378414489881L;
    ...
    public static void showLoadingMessage(){
    	final Graphics2D graphics2D = createGraphics();
		...
		new Thread(){
			...
			public void run() {
				...
				for(int i = 0; i<200; i++){
					...
					graphics2D.drawString("Lade Bilder....", 130, 200);
					graphics2D.setColor(new Color(80,110,180));
					graphics2D.draw3DRect(start,250, i,2, false);
					try {
						Thread.sleep(50);
    ...
    public static Graphics2D createGraphics(){
    	return (Graphics2D)getInstance().getGraphics();
    }
    ...
    public static void main(String[] args) {
    	URL url = ResourceAnchor.class.getResource("/resources/splashscreen.png");
    	System.out.println(url);
    	
        SplashWindow.splash(url);
        
        
        SplashWindow.showLoadingMessage();
